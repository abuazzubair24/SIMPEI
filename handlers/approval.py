...
async def approve_callback(update Update, context ContextTypes.DEFAULT_TYPE)
    ...
    await query.edit_message_caption(
        caption=(
            f📥 BUKTI PEMBAYARANnn
            f🆔 ID Pembayaran  {pembayaran_id}n
            f👤 Nama  {nama}n
            f✅ Status  APPROVEDnn
            f_Verified oleh admin_
        ),
        parse_mode=Markdown,
    )
    ...
    await context.bot.send_message(
        ...
        text=(
            f✅ Pembayaran Anda Telah Dikonfirmasinn
            fTerima kasih {nama} atas partisipasi Anda.nn
            fSemoga Allah ﷻ menerima dari kita semua. 
            fBarakallahu fiikum! 🤲
        ),
        parse_mode=Markdown,
    )
...
async def reject_callback(update Update, context ContextTypes.DEFAULT_TYPE)
    ...
    await context.bot.send_message(
        chat_id=chat_id_user,
        text=(
            f⚠️ Pembayaran Anda Ditolaknn
            fHalo {nama}, bukti transfer Anda tidak sesuai.nn
            fSilakan hubungi admin untuk penjelasan lebih lanjut.
        ),
        parse_mode=Markdown,
    )